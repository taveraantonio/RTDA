...
class SynthiaDataset(data.Dataset):
    ...
    def __init__(self, args, root, mean, crop_size, train=True, max_iters=None, ignore_index=255, file='./dataset/synthia_list/train.txt'):
        self.root = root
        self.mean = mean
        ...
        self.files = []
        if args.use_ss:
            self.img_ids = [i_id.strip() for i_id in open(file)]
        else:

...
class Visualizer:
    ...
    def __init__(self, args, rank=0, type='tensorboardX', debug=False, filename="logs2.txt", summary=True, step=None):
        self.logger = None
        self.args = args
        self.type = type
        self.rank = rank
    ...
    def display_current_results(self, visuals, step, mode, name):
        for k, (img, target, predicted) in enumerate(visuals):
            ...
            self.add_image(f'{mode}_{step}_{name}', concat_img)

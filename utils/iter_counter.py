...
class IterationCounter:
    ...
    def record_training_start(self, resume_iter=0, fine_tuning=False):
        self.training_end = False
        self.start_iter_time = time.time()
        self.total_steps_so_far = resume_iter
        self.last_iter_time = time.time()
        if fine_tuning:
            self.total_steps = self.args.fine_tuning_steps
    ...
    def needs_validating(self):
        if self.validate_epoch and self.epochs % self.args.eval_epoch == 0 and "source_only" not in self.args.train_mode:
            self.validate_epoch = False
            return True
        else:
